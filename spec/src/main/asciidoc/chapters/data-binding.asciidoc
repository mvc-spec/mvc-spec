...
Please note that usually `@MvcBinding` will be used with `@FormParam` bindings, as they are very common in web application.
However, depending on the specific use case, it may also be useful to use it with `@QueryParam` and other binding annotations.
