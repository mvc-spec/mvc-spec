...
~~~~~~~~~~~
// TODO insert links
An _MVC controller_ is a JAX-RS [insert] resource method decorated by `@Controller`. [insert] 
If this annotation is applied to a class, then all resource methods in it are regarded as controllers [insert]. 
...
void:: A controller method that returns void is REQUIRED to be decorated by `@View` [insert].
...
The default view MUST be used only when such a non-void controller method returns a `null` value [insert].
...
Likewise, injection of fields and properties is unrestricted and fully compatible with JAX-RS —modulo the restrictions explained in <<controller_instances,Controller Instances>>.

Controller methods handle a HTTP request directly. Sub-resource locators as described in the JAX-RS Specification [insert] are not supported by MVC.

[[controller_instances]]
Controller Instances
^^^^^^^^^^^^^^^^^^^^

Unlike in JAX-RS where resource classes can be native (created and managed by), CDI beans, managed beans or EJBs, MVC classes are REQUIRED to be CDI-managed beans only [insert]. 
...
Like in JAX-RS, the default resource class instance lifecycle is _per-request_ [insert]. 
...
See [insert] for more information on lifecycles and their caveats.

[[viewable]]
Viewable
^^^^^^^^

The `Viewable` class encapsulates information about a view as well as, optionally, information about how it should be processed. 
More precisely, a `Viewable` instance may include references to `Models` and `ViewEngine` objects —for more information
see Section [insert] and Chapter [insert], respectively. 
`Viewable` defines traditional constructors for all these objects and it is, therefore, not a CDI-managed bean.
...
`redirect:` as a more concise way to trigger a client redirect [insert]. 
...
For more information about view engines see Chapter [insert].
...
In a JSP, model properties are accessible via EL [insert]. In the example above, the property `message` is read from the `greeting` model 
whose name was either specified in a `@Named` annotation or used as a key in the `Models` map, depending on which controller from <<models, Modules>> triggered this view's processing.
