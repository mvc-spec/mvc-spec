...
a possible target for processing by calling its \code{supports} method, discarding the
...
required to package a \code{web.xml} deployment descriptor with the following entry
mapping the extension \code{*.xhtml} as shown next:
...
regular JSF post-backs will not be processed by the MVC runtime. The usage of \code{<h:form />}
and depending form components like \code{<h:inputText />} is not recommended as they would
