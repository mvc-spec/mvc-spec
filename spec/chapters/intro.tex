...
where it is used predominantly to build HTML-based applications. The {\em model} refers to the 
...
the majority of the controller logic is provided by framework instead of the application.

The API defined by this specification falls into the action-based 
category and is, therefore, not intended to be a replacement for component-based framworks like
...
\item[Non-Goal 2] Support for standalone implementations of \mvc\ running outside of Java EE.
\item[Non-Goal 3] Support for REST services not based on \jaxrs.
\item[Non-Goal 4] Provide built-in support for view languages that are not part of Java EE.
\end{description}

It is worth noting that, even though a standalone implementation of \mvc\ that can run outside
of Java EE is a non-goal, this specification shall not intentionally prevent implementations
to run in other environments, provided that they include support all the EE technologies
required by \mvc.
