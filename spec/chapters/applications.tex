...
This chapter introduces the notion of an \mvc\ application and how it relates to
a \jaxrs\ application.
...
also applies to an \mvc\ application. In what follows, we drop the adjective and 
refer to them simply as applications.
...
for the cases in which an \code{Application} subclass is present or not. 
...
convention, which this specification also recommends.

\section{Providers in \mvc}
\label{providers_in_mvc}

\mvc\ implementations are free use their own providers in order to modify the standard 
...
{\note Should we define the priorities for portability?}
