...
Given that redirects are fairly common in MVC frameworks, implementations MUST support
view paths prefixed by {\tt redirect:} as a more concise way to trigger a client redirect \assertref{redirect}. 
...
In either case, the HTTP status code returned is 302 and relative paths are resolved as
described in the Javadoc for method \code{seeOther} in \code{Response}.
Note that redirects require client cooperation and result in a completely new 
